...
def HarrisDetector(src, gaus_filter_size = 3, gaus_sigma = 1, alpha = 0.04, threshold_rate = 0.01):
    ...
    """
    gaus = my_get_Gaussian_filter((gaus_filter_size, gaus_filter_size), gaus_sigma)

    G_IxIx = GaussianFiltering(IxIx,gaus)
    G_IyIy = GaussianFiltering(IyIy,gaus)
    G_IxIy = GaussianFiltering(IxIy,gaus)
...
def main():
    src = cv2.imread('C:/Users/kimhyuck/Desktop/DSC/computer vision/4week/zebra.png')
    harris_img = src.copy()
    ...
    cv2.imshow('202101697 - interest points', interest_points)
    cv2.imshow('202101697 - harris_img', harris_img)
    cv2.waitKey()
